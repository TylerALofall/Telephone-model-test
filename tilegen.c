 ...
 * Each entry is 8 rows; each row is 8 bits.
 * Bit 7 = column 0 (leftmost), bit 0 = column 7 (rightmost).
 * A set bit means that cell is lit in the colour fill.          */
...
/* Character names used in filenames (must match the character itself) */
                       ...
                       const char *group, const char *item)
{
    ...
    if (n < 0 || (size_t)n >= sizeof(path)) {
        fprintf(stderr, "Path too long for dir='%s' colour='%s' group='%s' item='%s'\n",
                dir, colour, group, item);
        exit(EXIT_FAILURE);
    }
    FILE *f = fopen(path, "w");
...
static int write_balls(const char *dir, const char *colour, const char *hex)
{
    ...
    /* six outer at radius 150, angles 0,60,120,180,240,300 degrees */
    double angles[6] = {0.0, 60.0, 120.0, 180.0, 240.0, 300.0};
    for (int i = 0; i < 6; i++) {
        double a = angles[i] * M_PI / 180.0;
        int cx = iround(256.0 + 150.0 * cos(a));
        int cy = iround(256.0 + 150.0 * sin(a));
...
static int write_shapes(const char *dir, const char *colour, const char *hex)
{
    ...
    /* --- triangle --- */
    {
        ...
        write_svg_open(f);
        write_white_bg(f);
        /* 3 vertices on r=180 circle, start=-90°, step=120° */
        int px[3], py[3];
        for (i = 0; i < 3; i++) {
    ...
    /* --- pentagon --- */
    {
        ...
        write_svg_open(f);
        write_white_bg(f);
        /* 5 vertices on r=180 circle, start=-90°, step=72° */
        int px[5], py[5];
        for (i = 0; i < 5; i++) {
    ...
    /* --- hexagon --- */
    {
        ...
        write_svg_open(f);
        write_white_bg(f);
        /* 6 vertices on r=180 circle, start=-90°, step=60° */
        int px[6], py[6];
        for (i = 0; i < 6; i++) {
...
{
    if (argc != 2) {
        fprintf(stderr, "Usage: tilegen <colour>\n"
    ...
    if (!hex) {
        fprintf(stderr, "Unknown colour '%s'.\n"
                        "Valid colours: red orange yellow green blue violet brown black\n",
                word);
        return EXIT_FAILURE;
    }
    ...
    printf("colour: %s   hex: %s   files written: %d\n", word, hex, count);
